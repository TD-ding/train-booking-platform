...
router.post("/", authMiddleware, (req: Request, res: Response) => {
  ...
  const seatNumber = `${Math.floor(Math.random() * 15) + 1}车${Math.floor(Math.random() * 80) + 1}号`;
  ...
  const booking = db.prepare("SELECT * FROM bookings WHERE id = ?").get(bookingResult.lastInsertRowid);
...
router.post("/:id/pay", authMiddleware, (req: Request, res: Response) => {
  ...
  if (fromStop && toStop) {
    db.prepare(`
      ...
      AND from_stop_order <= ? AND to_stop_order >= ? AND available_seats > 0
